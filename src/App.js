...
function changeSelectedSubject(id, value) {
  setSelectedSubjectId(id);
  if(mode != 'delete' || deleteItem != "") {
    setMode('read');
  }
  setDeleteItem(value);
}
...
function deleteFunction(value) {
  let tempItem = Array.from(subject);
  let index = tempItem.findIndex(i=>i.id == value);
  tempItem.splice(index, 1);
  for(let i = index; i<tempItem.length; i++) {
    tempItem[i].id = tempItem[i].id - 1; 
  }
  setSubject(tempItem);
}
...
function getContent() {
  if(mode === 'welcome') {
    ...
    Viewer = <DeleteModeViewer deleteFunction = {deleteFunction} value = {subject}></DeleteModeViewer>
  }
  return Viewer;
}
